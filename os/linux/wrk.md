...
发起最简单请求
使用4个线程，模拟 20 个并发请求（也就是保持20个HTTP连接），连续测试10秒
...
注意：
wrk 的压测模式是基于事件的，而不是基于连接的。
也就是说，每个连接可以发送多个请求，而不是只发送一次。
wrk 会尽可能地在每个连接上发送请求，直到压测时间结束或者服务器无法响应。
所以，您的压测结果并不受限于连接数和线程数的乘积。
